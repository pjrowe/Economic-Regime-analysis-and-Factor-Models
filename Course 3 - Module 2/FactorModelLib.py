...
Week 2 - Machine learning techniques for robust estimation of factor models
    Comment - This is useful / good background to know, but generally too big
    picture for a small trader like myself who does not have access to
    massive amounts of market data and large portfolios to manage

Week 3 - Machine learning techniques for efficient portfolio diversification
    Comment - Same as Week 2 comment - Using powerful ML / data techniques for
        diversification is appropriate for massive portfolios.  Small
        dog like me will have to rely on feel and picking stocks /ETFs.

Week 4 - Machine learning techniques for regime analysis
    Comment - This is where it gets interesting.  My idea was that if I
        could analyze the same data everyone else does and see a
        recession coming, then maybe I would have time to make a
        conviction call on shorting the SP500 futures, which are easier
        to trade than individual equities.
Week 5 - Identifying recessions, crash regimes and feature selection
    Comment - Same as week 4 comment.  The idea is to use ML techniques
        to help inform my otherwise very seat of the pants approach.
        Since the market is so automated and dependent on ML now, then
        perhaps I need to be armed with the same tools.

        At least that was the idea.  Unfortunately, last year's crash was
        so fast and sharp, that it corrected based on Fed action within
        6 weeks.
        Thus, Monthly data is not quick enough to inform huge daily moves
        in the market.
"""
...
# follows the math, rather than in the restrictive standard form
# required by solvers."
...
def plot_returns(data, names, flag='Total Return', date='Date',
                 printFinalVals=False):
    """plot_returns returns a plot of the returns.

    INPUTS:
        names: string, name of column to be plotted, or list, in
            which case it plots all of them
    ...
    if (flag == 'Total Return'):
        ...
        totalReturns[0, :] = 1.
        for i in range(1, n):
            totalReturns[i, :] = np.multiply(totalReturns[i - 1, :],
                                             (1 + data[names].values[i, :]))
...
def create_options():
    """Create standard options dictionary.

    dictionary to be used as input to regression functions.
    """
    options = dict()
...
def create_options_best_subset():
    """Create options dictionary for input to regression functions."""
    options = create_options()
    options['returnModel'] = False
    options['printLoadings'] = True
    options['maxVars'] = 3
...
def create_dictionary_for_analysis(method, methodDict=None):
    """Create the options dictionary as an input to a factor model.

    INPUTS:
        method: string, defines the method
    OUTPUTS:
        methodDict: dictionary, keys are specific options the user wants
        to specify, values are the values of those options
    """
    if(method == 'OLS'):
        options = create_options()
    elif(method == 'CVLasso'):
...
def print_timeperiod(data, dependentVar, options):
    ...
    if(options['timeperiod'] == 'all'):
        ...
        print('Time period is between ' + num_to_month(beginDate.month)
              + ' ' + str(beginDate.year) + ' to '
              + num_to_month(endDate.month) + ' '
              + str(endDate.year) + ' inclusive   ')
    else:
        print('Time period is ' + options['timeperiod'])


def display_factor_loadings(intercept, coefs, factorNames, options):
    """Print the factor loadings in a readable way.
...
def best_subset(x, y, l_0):
    ...
    # cp.matmul is matrix multiplication; x*b where b is coeffs, x is
    # boolean feature vector, a is
    # intercept
    M = 1000
    n_factor = x.shape[1]
...
def linear_regression(data, dependentVar, factorNames, options):
    """Return the factor loadings using least squares regression.
    ...
    if (options['printLoadings']):
        ...
        display_factor_loadings(linReg.intercept_, linReg.coef_,
                                factorNames, options)
...
def lasso_regression(data, dependentVar, factorNames, options):
    """Return the factor loadings using lasso regression.
    ...
    INPUTS:
        ...
        NOTE: SKLearn calles Lambda Alpha.  Also, it uses a scaled
        version of LASSO argument, so here I scale when converting
        lambda to alpha
    ...
    if(options['timeperiod'] == 'all'):
        ...
        newData = data.copy()
        newData = newData.query(options['timeperiod'])
    lassoReg = Lasso(alpha=options['lambda'] / (2 * data.shape[0]),
                     fit_intercept=True)
    ...
    if (options['printLoadings']):
        ...
        display_factor_loadings(lassoReg.intercept_, lassoReg.coef_,
                                factorNames, options)
    if(options['returnModel']):
        return lassoReg
...
def ridge_regression(data, dependentVar, factorNames, options):
    """Return the factor loadings using ridge regression.
    ...
    if (options['printLoadings']):
        print_timeperiod(newData, dependentVar, options)
        print('lambda = ' + str(options['lambda']))
        display_factor_loadings(ridgeReg.intercept_, ridgeReg.coef_,
                                factorNames, options)
    if(options['returnModel']):
        return ridgeReg
...
def cross_validated_lasso_regression(data, dependentVar, factorNames, options):
    ...
    Cross validating the choice of lambda.
    ...
    if(options['randomState'] == 'none'):
        ...
        lassoTest = Lasso(random_state=options['randomState'],
                          fit_intercept=True)

    tuned_parameters = [{'alpha': alphas}]

    clf = GridSearchCV(lassoTest, tuned_parameters, cv=options['nFolds'],
                       refit=True)
    ...
    if (options['printLoadings']):
        ...
        display_factor_loadings(lassoBest.intercept_, lassoBest.coef_,
                                factorNames, options)
...
def cross_validated_ridge_regression(data, dependentVar, factorNames,
                                     options):
    """Return the factor loadings using ridge regression.

    ...choosing lambda via ridge regression
    INPUTS:
        data: pandas df, data matrix, should constain the date column
        and all of the factorNames columns
        dependentVar: string, name of dependent variable
        ...
        timeperiod, and date
            ...
            randomState: integer, sets random state seed
            nFolds: number of folds
            NOTE: SKLearn calles Lambda Alpha.  So I change
            Lambda -> Alpha in the following code
    Outputs:
        reg: regression object from sikitlearn
        also prints what was desired
    """
    ...
    if(options['randomState'] == 'none'):
        ...
        ridgeTest = Ridge(random_state=options['randomState'],
                          fit_intercept=True)
    tuned_parameters = [{'alpha': alphas}]

    clf = GridSearchCV(ridgeTest, tuned_parameters,
                       cv=options['nFolds'], refit=True)
    clf.fit(newData[factorNames], newData[dependentVar])
    ...
    if (options['printLoadings']):
        ...
        display_factor_loadings(ridgeBest.intercept_, ridgeBest.coef_,
                                factorNames, options)
    if(options['returnModel']):
        return ridgeBest


def cross_validated_elastic_net_regression(data, dependentVar,
                                           factorNames, options):
    """Return the factor loadings using elastic net, also chooses alpha.

    ...and l1 ratio via cross validation
    INPUTS:
        data: pandas df, data matrix, should constain the date column
        and all of the factorNames columns
        dependentVar: string, name of dependent variable
    ...
    if(options['randomState'] == 'none'):
        ...
        elasticNetTest = ElasticNet(random_state=options['randomState'],
                                    fit_intercept=True)

    tuned_parameters = [{'alpha': alphas, 'l1_ratio': l1Ratios}]

    clf = GridSearchCV(elasticNetTest, tuned_parameters, cv=options['nFolds'],
                       refit=True)
    clf.fit(newData[factorNames], newData[dependentVar])
    ...
    if (options['printLoadings']):
        print_timeperiod(newData, dependentVar, options)
        print('best lambda1 = '
              + str(alphaBest * 2 * newData.shape[0] * l1RatioBest))
        print('best lambda2 = '
              + str(newData.shape[0] * alphaBest * (1 - l1RatioBest)))
        display_factor_loadings(elasticNetBest.intercept_,
                                elasticNetBest.coef_, factorNames, options)
...
def relaxed_lasso_regression(data, dependentVar, factorNames, options):
    ...
    ...and cross validating the choice of lambda
    INPUTS:
        data: pandas df, data matrix, should constain the date column
        and all of the factorNames columns
        dependentVar: string, name of dependent variable
        ...
        timeperiod, and date
            ...
            randomState: integer, sets random state seed
            nFolds: number of folds
            NOTE: SKLearn calles Lambda Alpha.  Also, it uses a
            scaled version of LASSO argument, so here I scale
            when converting lambda to alpha
    Outputs:
        reg: regression object from sikitlearn
        also prints what was desired
    """
    ...
    if(options['CV']):
        lassoModel = cross_validated_lasso_regression(data, dependentVar,
                                                      factorNames, optionsNew)
    else:
        lassoModel = lasso_regression(data, dependentVar,
                                      factorNames, optionsNew)
    ...
    # Step 3: Run OLS using just the non-zero coeficients from the LASSO
    # regression
    olsReg = linear_regression(data, dependentVar, listNonZeroLoadings,
                               optionsNew)
    ...
    for i in range(len(factorNames)):
        if(lassoModel.coef_[i] != 0):
            ...
            coefs[i] = (optionsNew['gamma'] * lassoModel.coef_[i]
                        + (1 - optionsNew['gamma']) * olsReg.coef_[ind])

    reg = LinearRegression()
    reg.coef_ = coefs
    reg.intercept_ = (optionsNew['gamma'] * lassoModel.intercept_
                      + (1 - optionsNew['gamma']) * olsReg.intercept_)
    ...
    if (options['printLoadings']):
        ...
        if(options['CV']):
            print('best lambda = '
                  + str(lassoModel.alpha * 2 * newData.shape[0]))
        else:
            print('lambda = ' + str(options['lambda']))
        # Now print the factor loadings
        display_factor_loadings(reg.intercept_, reg.coef_,
                                factorNames, options)
    return reg


def run_factor_model(data, dependentVar, factorNames, method, options):
    """Return a model object according to the method chosen.

    INPUTS:
        data: pandas df, must contain the columns specified in
        factorNames and dependentVar
        dependentVar: string, dependent variable
        factorNames: list of strings, names of independent variables
        method: string, name of method to be used.  Supports OLS, LASSO,
        CVLASSO atm
        options: dictionary object, controls the hyperparameters of the
        method
    Outputs:
        out: model object
    """
    ...
    if (method == 'CVLasso'):
        return cross_validated_lasso_regression(data, dependentVar,
                                                factorNames, options)
    if (method == 'CVRidge'):
        return cross_validated_ridge_regression(data, dependentVar,
                                                factorNames, options)
    if (method == 'CVElasticNet'):
        return cross_validated_elastic_net_regression(data, dependentVar,
                                                      factorNames, options)
    if (method == 'BestSubset'):
        return best_subset_regression(data, dependentVar, factorNames,
                                      options)
    if (method == 'RelaxedLasso'):
        return relaxed_lasso_regression(data, dependentVar, factorNames,
                                        options)
    else:
        print('Method ' + method + ' not supported')


def compute_trailing_factor_regressions(data, dependentVar, factorNames,
                                        window, method, options,
                                        dateCol='Date', printTime=False):
    """Create a time series of factor loadings using a trailing window.

    Returns a pandas df object.

    INPUTS:
        data: pandas df, must constain the columns dependentVar, and
        the set of columns factorNames
        dependentVar: string, names the dependent variable, must be a
        column in the dataframe data
        ...
        printTime: boolean, if True, prints time it took to run the
        regressions
    Outputs:
        regressionValues: pandas df, rows should be different dates,
        columns should be factor loadings calculated using the trailing
        window
    """
    if(printTime):
        start = time.time()
    ...
    for i in range(window, len(days)):
        # Filter the data
        filtered = data[(data[dateCol] <= days[i])
                        & (data[dateCol] >= days[i - window])]
        # Run the regression
        reg = run_factor_model(filtered, dependentVar, factorNames,
                               method, options)
    ...
    if(printTime):
        print('regression took ' + str((time.time() - start) / 60.)
              + ' minutes')
    return regressionValues


def num_to_month(month):
    # Returns the name of the month when input is integer
    if (month == 1):
        return 'January'
    if (month == 2):
        return 'Febuary'
...
def data_time_periods(data, dateName):
    """Figure out if the data is daily, weekly, monthly, etc.
    ...
    thingy = ((last.values - secondToLast.values).astype('timedelta64[D]')
              / np.timedelta64(1, 'D'))
    thingy = thingy[0]
    if (thingy > 200):
        return 'yearly'
