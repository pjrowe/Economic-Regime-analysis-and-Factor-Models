...
def regime_plot(asset_data, column_num, regime_col):
    asset_return, ret_g, ret_c = regime_return(asset_data, [0,1,2,3,4,5,6,7], regime_col)
...
def trend_filtering(data, lambda_value):
    '''Runs trend-filtering algorithm to separate regimes ; the betas are can be turned into square wave by testing against threshhold level
    with regime_switch() function
        data: numpy array of total returns.
        
        <Phil Rowe>: 
        beta.value output is an array of length n that has values that minimize the problem; regime_switch function below 
        detects at which indices of beta.value there is a change in regime by testing against a threshold
        '''
    n = np.size(data)
...
def filter_plot(data, lambda_value, regime_num=0, TR_num=1):
    """Plots original data vs. filtered data; in our particular application, the original data was monthly returns of SP500
    and the filtered data (passed throuh trend_filtering() function was a set of beta values, or 'regimes', of the original data.
    The ‘fitted’ time series serves as the signal of the trend, and is an alternative approach to detecting the economic regime
    that tries to avoid the problems with timing / noise that result from trying to predict economic regime changes with macroeconomic data
    """


    ret_sp = data.iloc[:,regime_num]            # monthly returns of sp500
    sp500TR = data.values[:,TR_num]             # not used; not sure why we need; this is actual index level
...
def regime_switch(betas, threshold=1e-5):
    ...
    init_points.append(n)  # regimelist[-1] will be the length of betas
    return init_points
...
def plot_regime_color(dataset, regime_num=0, TR_num=1, lambda_value=16, log_TR = True):
    ...
    regimelist = regime_switch(betas)  # regimelist[-1] will be the length of betas; this is used to normalize x axis to 1
    ...
    if log_TR:
        ...
        plt.plot(TR, label='Total Return')      # plot sp500 value, which is in TR  
        plt.ylabel('SP500 Log-scale')
        plt.xlabel('Year')
        plt.yscale('log')
        plt.xlim([dataset.index[0], dataset.index[-1]])
        plt.ylim([80, 3500])
        plt.yticks([100, 500, 1000, 2000, 3000,3500], [100, 500, 1000, 2000, 3000,3500])
...
def geo_return(X, input_type='Return'):
    ...
    # y will have same dimensions as X, but each successive month of y will be cumulative return
    # hence, to get average per month return, we take cum return (very last row of
    # y, andtake root of nth power where n = # of timesteps
    
    else:       # this works if X entries are asset balances or values at each time step, rather than # returns
                # it returns the monthly geometric mean (assuming the X.shape is monthly as well )
...
def efficient_frontier_traditional(r_annual, Q_all, r_bar, asset_names):
    ...
    n_asset = r_annual.size                      # this is 8 for original implementation, as there are 8 assets to choose
    weight_vec = np.zeros((len(r_bar),n_asset))  # there are 16 different potential portfolio returns we want on frontier
                                                 # there are 8 assets, so 16x8 is weights for each of 16 returns  
    risk_port = np.zeros(len(r_bar))             # 16x1 risk values for each of 16 potential portfolio returns (x,y)
    ret_port = np.zeros(len(r_bar))              # 16 return values for portfolio

    for i, r_curr in enumerate(r_bar):
        w_opt = cp.Variable(r_annual.size)  # 8x1, weight variables to solve below, minimizing risk with constraints
        ret_opt = r_annual.T*w_opt          # optimized return is the r_annual * optimized weights
        risk_opt = cp.quad_form(w_opt, Q_all)   # executes w_opt.T * Q_all * w_opt 
                                                # Total variance = weighting individual variances with w^2 
                                                #  = sum(w^2*Var for each asset)
                                                # (8x1).T * 8x8  * 8x1 ==> 1x1 = portfolio variance
        prob = cp.Problem(cp.Minimize(risk_opt),        # finds weights that minimizes risk and maximizes return
...
def efficient_frontier_comparison(r_annual, Q_all, r_bar, n_scenarios=10000):
    ...
    for i, r_curr in enumerate(r_bar):
        ...
        weight_vec[i,:] = w_opt.value
        ret_port[i] = ret_opt.value
        risk_port[i] = np.sqrt(risk_opt.value)   # risk for chart is standard deviation
    ...
    # the following generates n_scenarios of annual returns across all assets, each return being a normal random variable with mean
    #  equivalent to geometric mean calculated from historical data, and covariance = what we calculated from historical data
    # then we 
    r_all_1 = np.random.multivariate_normal(r_annual.reshape(n_asset), Q_all, n_scenarios)

    Q_1 = np.cov(r_all_1.T)           # np.cov takes needs each row to be a variable, and each column a single observation
                                      # hence we need to transpose r_all_1 
    mu_1 = np.mean(r_all_1, axis=0)   # takes mean across 10,000 samples in 10,000x8 array and returns 8 means
    print('Mean returns from',n_scenarios,' samples is ', np.around(mu_1, 4))
    print('Means calculated from historical ', np.around(r_annual.reshape(n_asset), 4))
...
def efficient_frontier_twoRegime(ret, ret_g, ret_c, r_bar, asset_names, n_scenarios = 10000):
    ...
    # two regime model has some fraction of the 10,000 scenarios being growth regime random variable
    # and remainder of 10,000 scenarios being a contraction regime random variable
    ...
    efficient_frontier_traditional(r_annual, Q_all, r_bar, asset_names)  # this plots traditional frontier
...
def regime_asset(n, mu1, mu2, Q1, Q2, p1, p2):
    s_1 = np.random.multivariate_normal(mu1, Q1, n).T  # mu1 is average returns (geometric) for assets during growth regime
                                                       # Q1 is covariance matrix during growth
    s_2 = np.random.multivariate_normal(mu2, Q2, n).T  # mu2 is average return (geometric) for assets during contraction regime
                                                        # Q2 is covariance matrix during contraction (historical)
    regime = np.ones(n)
    
    # p1 is assigned n1/(n1+n2), or Prob(stay in regime 1 when in regime 1)
    # if random number generator generates a # > p1, then it falls into 1-p1 territory, i.e., it's the case that we switch
    #    to regime 0 when currently in regime 1
    # p2 is assigned n3/(n3+n4), or Prob(switch to regime 1 when in 0)
    # in the same way, if random # is >p2, then it's calling for 1-p2, or case where we stay in regime 0 in next step
    ...
    for i in range(n-1):
        if regime[i] == 1:
            ...
            if np.random.rand() > p2:
                regime[i+1] = 0
    return (regime*s_1 + (1-regime)*s_2).T      # when regime[i]==1, then s_1 multivariate normal is generated for that 
                                                #    scenario and month
...
def transition_matrix(regime):
    ...
    by summing the over the 'history' of the regime, we can calculate probabilities
    n1/(n1+n2) = prob of staying   in regime 1  in next step when currently in regime=  1 ; this will be p1
    n2/(n1+n2) = prob of switching to regime -1 in next step when currently in regime=  1 ; = 1-p1
    n3/(n3+n4) = prob of switching to regime 1  in next step when currently in regime= -1 ; this will be p2
    n4/(n3+n4) = prob of staying   in regime -1 in next step when currently in regime= -1 ; = 1-p2
    """
    n1,n2,n3,n4 = 0, 0, 0, 0
