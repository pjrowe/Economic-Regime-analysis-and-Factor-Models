...
Source Database:
- M. McCracken and S. Ng "FRED-MD: A Monthly Database for Macroeconomic
...
"""
Following the steps below to clean data and make it ready for feature
selection process.
...
# There are a few rows with missing values but they are at the end of
# the dataset, so there are no missing months
# in dataset; 59 years and 10 months, starting 1/1/1959,
# ending 10/2018, or 718 months
...
# 710 columns vs. 118 data columns before adding lags, 5x118
# => 590+120=710

# ------------- ------------- ------------- ------------- -------------
# 3. Test stationarity of time series
"""Augmented Dickey-Fuller Test can be used to test for stationarity
in macroeconomic
time series variables. We will use adfuller function from statsmodels
module in Python.
"""
# check stationarity
...
for column in bigmacro.drop(['Date', 'Regime'], axis=1):
    ...
    if result[1] > threshold:
        ...
        # replaces values with diff between current and prior row value
bigmacro = bigmacro.dropna(axis=0)
